...
def get_plot(x_new, F, S, dF, dS):
    ...
    plt.title("График разности точного решения и интерполяции")

    plt.subplot(1, 2, 2)
    plt.plot(x_new, [abs(dF[i]-dS[i]) for i in range(len(dS))], 'b--')
    plt.title("График разности производных точного решения и интерполяции")
    plt.show()
...
def main_task(n):
    ...
    S, F, dF, dS = cubic_interpolation(h, border1, border2, f, x, x_new, '2')

    get_plot(x_new, F, S, dF, dS)
    get_info(n, x_new, [abs(F[i]-S[i]) for i in range(len(S))], \
        [abs(dF[i]-dS[i]) for i in range(len(dS))])
...
def test_task(n):
    ...
    S, F, dF, dS = cubic_interpolation(h, border1, border2, f, x, x_new, '1')
    get_plot(x_new, F, S, dF, dS)
    get_info(n, x_new, [abs(F[i]-S[i]) for i in range(len(S))], \
        [abs(dF[i]-dS[i]) for i in range(len(dS))])
...
def osc_task(n):
    ...
    S, F, dF, dS = cubic_interpolation(h, border1, border2, f, x, x_new, '3')
    get_plot(x_new, F, S, dF, dS)
    get_info(n, x_new, [abs(F[i]-S[i]) for i in range(len(S))], \
        [abs(dF[i]-dS[i]) for i in range(len(dS))])
